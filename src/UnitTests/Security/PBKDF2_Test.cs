...
public class Pbkdf2Test
{
    ...
    public void TestPbe()
    {
        ...
        rng.GetBytes(data);
        Rfc2898DeriveBytes rfc = new("test", data, 400000);
        _ = rfc.GetBytes(20);
        sw.Stop();
    ...
    public void TestPbe3()
    {
        ...
        byte[] data = new byte[30];
        rng.GetBytes(data);
        Pkcs5S2ParametersGenerator pk = new(new Sha512Digest());
    ...
    private void TestSha1(string p, string s, int c, int dkLen, string dk)
    {
        byte[] dkExpected = StringToByteArray(dk);

        using (Pbkdf2 pdf = new(HmacMethod.Sha1, Encoding.ASCII.GetBytes(p), Encoding.ASCII.GetBytes(s), c))
        {
    ...
    private void TestSha256(string p, string s, int c, int dkLen, string dk)
    {
        byte[] dkExpected = StringToByteArray(dk);

        using (Pbkdf2 pdf = new(HmacMethod.Sha256, Encoding.ASCII.GetBytes(p), Encoding.ASCII.GetBytes(s), c))
        {
            byte[] dkBytes = pdf.GetBytes(dkLen);
            if (!dkBytes.SequenceEqual(dkExpected))
                throw new Exception();
    ...
    private void TestSha512(string p, string s, int c, int dkLen, string dk)
    {
        byte[] dkExpected = StringToByteArray(dk);

        using (Pbkdf2 pdf = new(HmacMethod.Sha512, Encoding.ASCII.GetBytes(p), Encoding.ASCII.GetBytes(s), c))
        {
            byte[] dkBytes = pdf.GetBytes(dkLen);
            if (!dkBytes.SequenceEqual(dkExpected))
                throw new Exception();
